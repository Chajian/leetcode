...
public class b23 {
    ...
    public ListNode separate(ListNode[] lists,int start,int end){
        if(start == end) return lists[start];
        int distance = ((end-start)/2);
        ListNode left = separate(lists,start,start+distance);
        ListNode right = separate(lists,start+distance+1,end);
